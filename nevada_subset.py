...
# 0. SETTINGS
# --------------------------------------------------
SHAPEFILE_PATH = r"C:\Users\benne\Downloads\nv_2020\nv_2020.shp"

# Subset settings
USE_SUBSET = True
SUBSET_N = 50          # try 50 first, then 75, then 100
SUBSET_P = 3           # use 2 for 50 nodes, maybe 3 for 75-100
START_NODE = None      # set to a UNIQUE_ID manually if you want

# Population estimate parameters
turnout_rate = 0.67
vap_share = 0.70

# Districting settings
FULL_P = 4
POP_TOL = 0.10
...
# --------------------------------------------------
# 1. HELPER: BFS CONNECTED SUBSET
# --------------------------------------------------
def bfs_subset(graph, start, max_nodes):
    visited = set([start])
    q = deque([start])
...
# --------------------------------------------------
# 2. Read Nevada precinct/VTD shapefile
# --------------------------------------------------
gdf = gpd.read_file(SHAPEFILE_PATH)

# --------------------------------------------------
# 3. Drop Z dimension if needed
# --------------------------------------------------
...
print("Rows in gdf:", len(gdf), flush=True)
print("Unique UNIQUE_IDs:", gdf["UNIQUE_ID"].nunique(), flush=True)
assert gdf["UNIQUE_ID"].nunique() == len(gdf)

# --------------------------------------------------
# 6. Create vote columns
# --------------------------------------------------
...
print("Nevada units:", len(gdf_small), flush=True)

# --------------------------------------------------
# 9. Candidate adjacency pairs using intersects
# --------------------------------------------------
...
print("Candidate intersecting pairs:", len(cand), flush=True)

# --------------------------------------------------
# 10. Keep only pairs with positive shared boundary length
# --------------------------------------------------
...
print("Adjacency pairs with positive shared boundary:", len(edges), flush=True)
...
print("\nInitial graph:", flush=True)
print("Nodes:", G.number_of_nodes(), flush=True)
print("Edges:", G.number_of_edges(), flush=True)

isolated = list(nx.isolates(G))
print("Initial isolated nodes:", len(isolated), flush=True)
print("First few isolated nodes:", isolated[:20], flush=True)

components = list(nx.connected_components(G))
sizes = sorted([len(c) for c in components], reverse=True)

print("Initial connected components:", len(components), flush=True)
print("Largest component size:", max(len(c) for c in components), flush=True)
print("Largest 20 component sizes:", sizes[:20], flush=True)

# --------------------------------------------------
# 12. Keep only the largest connected component
# --------------------------------------------------
largest_cc = max(nx.connected_components(G), key=len)

dropped_nodes = set(G.nodes()) - set(largest_cc)
print("\nDropping nodes outside largest component:", len(dropped_nodes), flush=True)
print("First few dropped nodes:", list(dropped_nodes)[:20], flush=True)
...
print("\nAfter keeping largest connected component:", flush=True)
print("Nodes:", G.number_of_nodes(), flush=True)
print("Edges:", G.number_of_edges(), flush=True)
print("Connected components:", nx.number_connected_components(G), flush=True)
print("Is connected?", nx.is_connected(G), flush=True)

# --------------------------------------------------
# 13. OPTIONAL: Build a smaller connected subset
# --------------------------------------------------
if USE_SUBSET:
    if START_NODE is None:
        START_NODE = next(iter(G.nodes()))

    subset_nodes = bfs_subset(G, START_NODE, SUBSET_N)

    print("\nBuilding connected subset...", flush=True)
    print("Start node:", START_NODE, flush=True)
    print("Requested subset size:", SUBSET_N, flush=True)
    print("Actual subset size:", len(subset_nodes), flush=True)

    # filter dataframe
    gdf_small = gdf_small[gdf_small["UNIQUE_ID"].isin(subset_nodes)].copy()

    # filter edges
    edges = edges[
        edges["u"].isin(subset_nodes) & edges["v"].isin(subset_nodes)
    ].copy()
    ...
    print("Subset graph nodes:", G.number_of_nodes(), flush=True)
    print("Subset graph edges:", G.number_of_edges(), flush=True)
    print("Subset connected?", nx.is_connected(G), flush=True)
...
print("\nReady for optimization:", flush=True)
print("Number of precincts:", len(precincts), flush=True)
print("Unique precincts:", len(set(precincts)), flush=True)
assert len(precincts) == len(set(precincts))
print("Number of undirected edges:", len(edges_list), flush=True)
print("Number of directed arcs:", len(arcs), flush=True)
print("Number of districts p:", p, flush=True)

# Size estimates
print("\nEstimated model size:", flush=True)
print("x vars ~", len(precincts) * len(precincts), flush=True)
print("f vars ~", len(arcs) * len(precincts), flush=True)

total_pop = sum(pop[i] for i in precincts)
target_pop = total_pop / p
L = (1 - POP_TOL) * target_pop
U = (1 + POP_TOL) * target_pop

print("\nPopulation targets:", flush=True)
print("Total population:", total_pop, flush=True)
print("Target population:", target_pop, flush=True)
print("Population lower bound:", L, flush=True)
print("Population upper bound:", U, flush=True)
...
print("Total Republican votes:", total_rep_votes, flush=True)
print("Total Democratic votes:", total_dem_votes, flush=True)
print("Total two-party votes:", total_votes, flush=True)
